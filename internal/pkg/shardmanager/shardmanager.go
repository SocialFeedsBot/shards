...
func (m *Manager) statusRoutine() {
	...
	if err != nil {
		...
		for _, msg := range msgs {
			...
			embed := msg.Embeds[0]
			if embed.Title == m.Name+" Shard Status" {
...
func (m *Manager) updateStatusMessage(mID string) (string, error) {
	...
	for _, shard := range status.Shards {
		...
		content += fmt.Sprintf("[SHARD %d/%d]: %s (%d servers)\n", shard.Shard, m.numShards, emoji, shard.NumGuilds)
	}

	embed := &discordgo.MessageEmbed{
		Title:       m.Name + " Shard Status",
		Description: content,
		Color:       0x4286f4,
...
var (
	...
	eventColors = map[EventType]int{
		EventOpen:         0xec58fc,
		EventClose:        0xff7621,
		EventConnected:    0x77dd77,
		EventDisconnected: 0xcc2424,
		EventResumed:      0xffb347,
		EventReady:        0xaec6cf,
		EventError:        0xff6961,
	}
)
